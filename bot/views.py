...
from .models import (
    ...
    Order_items,
    Working)
from .credentials import API_ENDPOINT, URL, THE_HOST_URL
from .trans import t
...
def sendProductDetail(message, data, object_from_database):
    ...
    for i in object_from_database:
        if str(i) == data:
            print(THE_HOST_URL + f"/media/{i.image}")
            bot_request('sendPhoto', {
                'chat_id': user_id,
                'photo': THE_HOST_URL + f"/media/{i.image}",
...
def editProductDetailTest_withAddToCart(**kwargs):
    ...
    print(THE_HOST_URL + f"/media/{kwargs['productObj'].image}")
    bot_request('editMessageCaption', {
        'chat_id': user_id,
        'message_id': message_id,
        'photo': THE_HOST_URL + f"{kwargs['productObj'].image}",
        'caption': 
...
def sendProductDetailTest(message, data, object_from_database, count_num=0, check_first_time=True, message_with_inline=None):
    ...
    print(THE_HOST_URL + f"{ovqat.image}")
    bot_request('sendPhoto', {
                'chat_id': user_id,
                'photo': THE_HOST_URL + f"/media/{ovqat.image}",
